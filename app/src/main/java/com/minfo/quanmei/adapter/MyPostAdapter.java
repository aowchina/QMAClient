...
public class MyPostAdapter extends BaseAdapter {
    ...
    public MyPostAdapter(Context context, List<GroupArticle> list) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        viewHolder.name.setText(diary.getPlusername());
        viewHolder.upLoadTime.setText(diary.getPltime());
        viewHolder.handCount.setText(diary.getZan());
        viewHolder.shareCount.setText(diary.getPl());
        viewHolder.body.setText("[" + diary.getGname() + "]");

        if (diary.getImgs()!=null&&diary.getImgs().size()!=0) {
            for (int i = 0; i < diary.getImgs().size(); i++) {
                if (i == 0) {
                    UniversalImageUtils.displayImageUseDefOptions(diary.getImgs().get(i), viewHolder.iconA);
                } else if (i == 1) {
                    UniversalImageUtils.displayImageUseDefOptions(diary.getImgs().get(i), viewHolder.iconB);
                }
            }
        }else{
        ...
        UniversalImageUtils.disCircleImage(diary.getPluserimg(), viewHolder.useIcon);
