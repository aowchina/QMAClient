package com.minfo.quanmei.chat.domain;
...
public class EaseAtMessageHelper {
    ...
    public void parseMessages(List<EMMessage> messages) {
        ...
        for(EMMessage msg : msgs){
            if(msg.getChatType() == ChatType.GroupChat){
                ...
                try {
                    JSONArray jsonArray = msg.getJSONArrayAttribute(Constant.MESSAGE_ATTR_AT_MSG);
                    ...
                    String usernameStr = msg.getStringAttribute(Constant.MESSAGE_ATTR_AT_MSG, null);
                    if(usernameStr != null){
                        if(usernameStr.toUpperCase().equals(Constant.MESSAGE_ATTR_VALUE_AT_MSG_ALL)){
                            if(!atMeGroupList.contains(groupId)){
                                atMeGroupList.add(groupId);
                            }
                        }
    ...
    public boolean isAtMeMsg(EMMessage message){
        ...
        if(user != null){
            try {
                JSONArray jsonArray = message.getJSONArrayAttribute(Constant.MESSAGE_ATTR_AT_MSG);
                ...
                String atUsername = message.getStringAttribute(Constant.MESSAGE_ATTR_AT_MSG, null);
                if(atUsername != null){
                    if(atUsername.toUpperCase().equals(Constant.MESSAGE_ATTR_VALUE_AT_MSG_ALL)){
                        return true;
                    }
                }
                return  false;
